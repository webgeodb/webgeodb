...
基于项目内部 benchmark 测试（10,000 条 POI 数据，MacBook Pro M1）：

```
┌─────────────────────────┬──────────────┬─────────────┐
│ 操作                    │ 无空间索引   │ 有空间索引  │
├─────────────────────────┼──────────────┼─────────────┤
│ 距离查询（10K）         │ ~150ms       │ ~8-15ms     │
│ 相交查询（10K）         │ ~180ms       │ ~9-18ms     │
│ 包含查询（10K）         │ ~200ms       │ ~10-20ms    │
└─────────────────────────┴──────────────┴─────────────┘
```

**性能优化**：
- R-Tree 空间索引（动态数据）
- Flatbush 静态索引（只读数据，快约 2x）
- LRU 查询缓存
- BBox 预检查优化（不相交对象早期退出）
...
以下数据来自 WebGeoDB 自身的 benchmark 测试（`vitest bench`），测试环境：
...
在有无空间索引的情况下，同一个 WebGeoDB 实例对比（10K 数据量）：

```
┌──────────────┬──────────────┬───────────────┬────────┐
│ 查询类型     │ 无索引（全扫）│ R-Tree 索引   │ 静态索引│
├──────────────┼──────────────┼───────────────┼────────┤
│ 距离查询     │ ~150ms       │ ~15ms         │ ~8ms   │
│ 包含查询     │ ~200ms       │ ~20ms         │ ~10ms  │
│ 相交查询     │ ~180ms       │ ~18ms         │ ~9ms   │
└──────────────┴──────────────┴───────────────┴────────┘
```

> 数据来源：`packages/core/test/benchmark/` 基准测试，实际数值因机器和数据分布有所浮动。

**结论**：
- ✅ 加上空间索引后，查询速度提升 **10-20 倍**
- ✅ 静态索引（Flatbush）适合只读数据，比 R-Tree 再快约 2 倍
- ✅ 空间谓词经过优化后，单次调用均在 **1ms 以内**

### 5.3 优化谓词的提升

WebGeoDB 在 Turf.js 基础上实现了 BBox 预检查等优化：

```
┌─────────────────────────┬─────────────┬─────────────┐
│ 操作                    │ 原始引擎    │ 优化版本    │
├─────────────────────────┼─────────────┼─────────────┤
│ 简单几何（点-点/点-线） │ < 5ms/次    │ < 1ms/次    │
│ 复杂几何（面-面）       │ < 30ms/次   │ < 10ms/次   │
│ 不相交对象（早期退出）  │ 正常耗时    │ 显著更快    │
│ 批量操作（1000个点）    │ 基准        │ 快 50-70%   │
└─────────────────────────┴─────────────┴─────────────┘
```

### 5.4 包大小对比

包大小是 WebGeoDB 的核心优势之一（实际构建产物）：

```
SQLite WASM（sql.js）:  ~1,000 KB
SQLite + SpatiaLite:    ~2,000 KB+
WebGeoDB:               ~200 KB    ✅ 小 80%
```

> **说明**：目前尚未对 SQLite WASM 等其他方案做系统性的横向基准对比，上述查询性能数据均为 WebGeoDB 内部测试结果。欢迎社区提交对比测试报告。
